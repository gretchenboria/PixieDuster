...
def arrow_right(d: Doc, x1, y, x2, *, colour=DIM_GOLD) -> None:
...
def heading(d: Doc, y: float, title: str, sub: str) -> float:
    text(d, MARGIN, y, CW, title, size=19, style="B", colour=GOLD, lh=9)
    text(d, MARGIN, y + 10, CW, sub, size=9.6, colour=LILAC, lh=5)
...
def page_flow(d: Doc) -> None:
    ...
    text(d, MARGIN, y, CW, "1.  EVIDENCE", size=9, style="B", colour=GOLD, lh=5)
    y += 6
    bw = (CW - 8) / 3
    inputs = [
    ...
    for i, (label, flag, detail, tint) in enumerate(inputs):
        ...
        text(d, bx + 3, y + 3.5, bw - 6, label, size=9.4, style="B", colour=tint,
             align="C", lh=4.6)
        mono(d, bx + 3, y + 9, bw - 6, flag, size=7.4, colour=GOLD, align="C")
        text(d, bx + 3, y + 15, bw - 6, detail, size=7.2, colour=MAUVE, align="C", lh=3.6)
    ...
    text(d, MARGIN + 5, y + 3, CW - 10, "2.  LOCAL PROCESSING    (nothing has left your machine yet)",
         size=9, style="B", colour=GOLD, lh=4.6)
    steps = [
    ...
    for i, label in enumerate(steps):
        ...
        text(d, bx, y + 13.5, sw, label, size=6.8, colour=LILAC, align="C", lh=3.4)
    ...
    text(d, MARGIN, y + 5.5, CW, "network boundary - --dry-run stops here",
         size=6.6, colour=ROSE, align="R", lh=3.4)
    ...
    for i, (label, detail) in enumerate(crit):
        ...
        text(d, bx + 1, y + 13.5, cw2 - 2, label, size=7.6, style="B", colour=GOLD,
             align="C", lh=3.6)
        text(d, bx + 1, y + 24, cw2 - 2, detail, size=6.4, colour=MAUVE, align="C", lh=3.2)
    ...
    text(d, MARGIN + 5, y + 3.5, CW - 10, "4.  ONE FILE", size=9, style="B",
         colour=GREEN, lh=4.6)
    mono(d, MARGIN + 5, y + 9, CW - 10, "persona.md", size=11, colour=GOLD)
    text(d, MARGIN + 5, y + 15.5, CW - 10,
         ...
         size=7.6, colour=MAUVE, lh=3.7)

    y += 30
    text(d, MARGIN, y, CW, "What the persona file actually controls",
         size=10, style="B", colour=LILAC, lh=5.5)
    y += 7
    controls = [
    ...
    for i, item in enumerate(controls):
        ...
        text(d, bx + 5, by, CW / 3 - 6, item, size=8, colour=MAUVE, lh=4)
...
def page_science(d: Doc) -> None:
    ...
    blocks = [
        ...
        ("Benign Violation Theory", "McGraw - what the humour slider actually sets",
    ...
    for title, sub, body, example, tint in blocks:
        ...
        text(d, MARGIN + 5, y + 3.5, CW - 10, title, size=10.4, style="B", colour=tint, lh=5)
        text(d, MARGIN + 5, y + 9.5, CW - 10, sub, size=7.2, colour=FAINT, lh=3.6)
        text(d, MARGIN + 5, y + 14.5, CW - 10, body, size=8.2, colour=MAUVE, lh=4.1)

        panel(d, MARGIN + 5, y + 30, CW - 10, 11, fill=(18, 11, 32), edge=tint,
              radius=1.5, lw=0.25)
        mono(d, MARGIN + 8, y + 32, CW - 16, example, size=6.6, colour=LILAC)
...
def page_stack(d: Doc) -> None:
    ...
    for layer, choice, why in rows:
        text(d, xs[0], hy + 1.5, (CW - 10) * widths[0] - 3, layer, size=8, colour=LILAC, lh=4)
        d.set_font("Courier", "", 7.8)
        d.set_text_color(*GOLD)
        d.set_xy(xs[1], hy + 1.5)
        d.multi_cell((CW - 10) * widths[1] - 3, 4, choice)
        text(d, xs[2], hy + 1.5, (CW - 10) * widths[2] - 3, why, size=8, colour=MAUVE, lh=4)
        hy += 9.4

    y += 12 + len(rows) * 9.4 + 8

    # --- what it does NOT do ----------------------------------------------
    text(d, MARGIN, y, CW, "Deliberately absent", size=10.4, style="B", colour=LILAC, lh=5.5)
    y += 7
    absent = [
    ...
    for title, body in absent:
        ...
        text(d, MARGIN + 5, y + 2.5, CW - 10, title, size=8.6, style="B", colour=GREEN, lh=4.2)
        text(d, MARGIN + 5, y + 7.2, CW - 10, body, size=7.8, colour=MAUVE, lh=3.8)
        y += 15

    y += 4
    panel(d, MARGIN, y, CW, 26, fill=(40, 24, 30), edge=ROSE, lw=0.45)
    text(d, MARGIN + 5, y + 3, CW - 10, "The one thing that leaves your machine",
         size=8.6, style="B", colour=ROSE, lh=4.2)
    text(d, MARGIN + 5, y + 8, CW - 10,
         "Your writing goes to Google's Gemini API to be analysed. That is the product, not a "
