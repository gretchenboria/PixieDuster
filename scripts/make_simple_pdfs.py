...
def text(d: Doc, x, y, w, s, *, size=10, style="", colour=MAUVE, align="L", lh=5.0) -> float:
...
def mono(d: Doc, x, y, w, s, *, size=9.5, colour=GOLD, align="L") -> None:
...
def down_arrow(d: Doc, x, y1, y2, *, colour=DIM_GOLD) -> None:
...
def diagram() -> Path:
    ...
    for i, (label, how) in enumerate(ways):
        ...
        text(d, bx, y + 24.5, bw, label, size=8.2, style="B", colour=LILAC,
             align="C", lh=3.8)
        mono(d, bx + 2, y + 29.5, bw - 4, how, size=5.6, colour=GOLD, align="C")
    ...
    text(d, MARGIN + 5, y + 11, half - 10,
         ...
         'testament to..."', size=9, colour=MAUVE, lh=4.6)
    ...
    text(d, MARGIN + half + 11, y + 11, half - 10,
         ...
         "   - Bolt, a desktop robot, humour set to 8.", size=9, colour=MAUVE, lh=4.6)

    # --- footer -----------------------------------------------------------
    d.set_draw_color(*DIM_GOLD)
    d.set_line_width(0.4)
    d.line(MARGIN + 45, 275, PAGE_W - MARGIN - 45, 275)
    text(d, 0, 279, PAGE_W, "One command does all of it:", size=9, colour=FAINT,
         align="C", lh=4.5)
    mono(d, 0, 284, PAGE_W, "pixieduster clone", size=12, colour=GOLD, align="C")
...
def step(d: Doc, y: float, n: str, title: str, body: str, cmd: str | None = None,
    ...
    text(d, MARGIN + 18, y + 5.5, CW - 26, title, size=11.5, style="B", colour=GOLD, lh=5.5)
    inner = text(d, MARGIN + 18, y + 13, CW - 26, body, size=9.4, colour=MAUVE, lh=4.7)
    ...
    if note:
        text(d, MARGIN + 18, inner + 0.5, CW - 26, note, size=8.4, colour=FAINT, lh=4.2)
...
def quickstart() -> Path:
    ...
    text(d, 0, 33, PAGE_W, "Quick start", size=22, style="B", colour=GOLD,
         align="C", lh=9)
    text(d, MARGIN, 45, CW, "Five minutes, start to finish.", size=11,
         colour=LILAC, align="C", lh=5.5)
    ...
    y = step(d, y, "4", "Run it for real",
             "Answer three questions with the arrow keys, set the humour slider, wait "
    ...
    text(d, MARGIN, 22, CW, "What you will see", size=16, style="B", colour=GOLD, lh=8)
    y = 34.0
    ...
    for line in seen:
        ...
        text(d, MARGIN + 13, yy, CW - 22, line, size=9.4, colour=MAUVE, lh=4.6)
        yy += 6.4

    y = 84.0
    text(d, MARGIN, y, CW, "The other two commands", size=16, style="B", colour=GOLD, lh=8)
    y += 12
    for cmd, what in (
        ...
        text(d, MARGIN + 6, y + 10, CW - 12, what, size=9, colour=FAINT, lh=4.4)
        y += 21

    y += 4
    text(d, MARGIN, y, CW, "If something goes wrong", size=16, style="B", colour=GOLD, lh=8)
    y += 12
    trouble = [
    ...
    for title, body in trouble:
        ...
        text(d, MARGIN + 6, y + 3.5, CW - 12, title, size=9.4, style="B", colour=LILAC, lh=4.6)
        text(d, MARGIN + 6, y + 9, CW - 12, body, size=8.8, colour=MAUVE, lh=4.3)
        y += h + 4
    ...
    text(d, 0, 273, PAGE_W,
         "Your writing is sent to Google to be analysed. Use --dry-run to see exactly "
         "what would go, before it goes.", size=8.6, colour=FAINT, align="C", lh=4.3)
